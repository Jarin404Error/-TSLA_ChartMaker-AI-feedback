...
st.set_page_config(
    page_title="TSLA Levels Dashboard",
    page_icon="⚡",
    layout="wide"
)
...
# --- 3. Helper Functions (Copied from your script) ---
...
def get_level_colors():
    ...
    return {
        ...
        "ORB_5/15_High": "cyan",  # <-- NEW MERGED COLOR
        "ORB_5/15_Low": "cyan",   # <-- NEW MERGED COLOR
    }

@st.cache_data(ttl=300) # Cache data for 5 minutes (300 seconds)
def fetch_data(ticker):
    """Fetches the last 2 days of 5-min data for a ticker."""
...
def plot_chart(df, levels):
    ...
    # 3. Add Level Lines (with your new merge logic)
    
    # --- Pre-process levels to merge overlapping ORBs ---
    plot_levels = levels.copy() # Start with a copy of all levels
    ...
    if orb_5_high is not None and orb_15_high is not None and orb_5_high == orb_15_high:
        print(f"Merging ORB_5_High and ORB_15_High at {orb_5_high:.2f}")
        plot_levels["ORB_5/15_High"] = orb_5_high
        del plot_levels["ORB_5_High"]
        del plot_levels["ORB_15_High"]
    ...
    if orb_5_low is not None and orb_15_low is not None and orb_5_low == orb_15_low:
        print(f"Merging ORB_5_Low and ORB_15_Low at {orb_5_low:.2f}")
        plot_levels["ORB_5/15_Low"] = orb_5_low
        del plot_levels["ORB_5_Low"]
        del plot_levels["ORB_15_Low"]
    # --- End of new pre-processing logic ---

    colors = get_level_colors()
    
    # Loop over the MODIFIED plot_levels dictionary
    ...
    fig.update_layout(
        ...
        xaxis_title="Time",
        yaxis_title="Price",
        height=700, # Adjusted height for a web app
        showlegend=False,
    ...
    # --- IMPORTANT: Return the figure ---
    return fig, plot_levels

# --- 4. Main App Logic ---

st.title(f"{TICKER} Intraday Levels Dashboard")

# Fetch data
data = fetch_data(TICKER)

if data.empty:
    st.error("No data found. Is the market open or is this a holiday?")
else:
    # Calculate levels
    ...
    if data_filtered.empty:
        ...
        # Call your modified plotting function
        fig, processed_levels = plot_chart(data_filtered, levels) 
        ...
        with st.sidebar:
            ...
            st.write(f"Last update: {dt.datetime.now(pytz.timezone(MARKET_TZ)).strftime('%I:%M:%S %p ET')}")
            
            st.markdown("---") # Divider
            
            st.subheader("Calculated Levels")
            # Sort the processed levels for display
        ...
        # --- 7. Display the Chart ---
        # This is the magic command to show your Plotly chart
        st.plotly_chart(fig, use_container_width=True)

        st.caption("This dashboard fetches 2 days of 5-minute data to calculate previous day and pre-market levels.")